...
func redraw_all() {
	...
	// tbprint(midx+6, midy+3, coldef, coldef, "Press ESC|Ctrl+Q to quit")
	// msg := 
	// tbprint(midx+5, midy+3, coldef, coldef, msg)
	for i, message := range messages {
		shift := (edit_box_width - RuneLen(message))/2
		tbprint(midx+shift, midy+3+i, coldef, coldef, message)
	}
	
	// tbprint(midx+shift, midy+3, coldef, coldef, message)
	termbox.Flush()
}
...
var arrowLeft = '←'
var arrowRight = '→'
var messages = []string{"Press Ctrl+C to quit"}
	...
	for {
		...
		case termbox.EventKey:
			...
			case termbox.KeyEnter:
				// message = Read(edit_box.text)
				msgs := []string{}
				msgs = append(msgs, Read(edit_box.text...))
